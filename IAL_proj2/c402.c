    ...
    while(true){
        if(*RootPtr){
            if(Content < (*RootPtr)->Cont){
                ...
                //Ukazatel již existuje, nic se neprovádí
                break;
            }
        }else{
            //Jedná se o nový strom nebo jsme došli na místo, kam vložit prvek
...
void BTPreorder (tBTNodePtr RootPtr)    {
    ...
    tStackP stack;
    SInitP(&stack);
    tBTNodePtr actualPtr = RootPtr;
    Leftmost_Preorder(actualPtr, &stack);
    
    while(!SEmptyP(&stack)){
        actualPtr = STopPopP(&stack);
        Leftmost_Preorder(actualPtr->RPtr, &stack);
    }
...
void BTInorder (tBTNodePtr RootPtr)    {
    ...
    SInitP(&stack);
    tBTNodePtr actualPtr = RootPtr;
    Leftmost_Inorder(actualPtr, &stack);
    
    while(!SEmptyP(&stack)){
        actualPtr = STopPopP(&stack);
        BTWorkOut(actualPtr);
        Leftmost_Inorder(actualPtr->RPtr, &stack);
    }
...
void BTPostorder (tBTNodePtr RootPtr)    {
    ...
    SInitB(&stackB);
    tBTNodePtr actualPtr = RootPtr;
    Leftmost_Postorder(actualPtr, &stackP, &stackB);
    
    while(!SEmptyP(&stackP)){
        actualPtr = STopPopP(&stackP);
        if(STopPopB(&stackB)){
            SPushB(&stackB, false);
            SPushP(&stackP, actualPtr);
            Leftmost_Postorder(actualPtr->RPtr, &stackP, &stackB);
        }else{
            BTWorkOut(actualPtr);
        }
    }
